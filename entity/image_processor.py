    ...
    def process(self, container: ImageContainer) -> None:
        ...
        if self.logo_enable:
            if self.is_logo_left():
                ...
                append_image_by_side(watermark, [line, logo, left], padding=280, is_start=logo is None)
                append_image_by_side(watermark, [right], side='right', padding=280)
            else:
                # 如果 logo 在右边
                if logo is not None:
                    ...
                    line = LINE_TRANSPARENT.copy()
                append_image_by_side(watermark, [left], padding=280, is_start=True)
                append_image_by_side(watermark, [logo, line, right], side='right', padding=280)
                line.close()
        else:
            append_image_by_side(watermark, [left], padding=280, is_start=True)
            append_image_by_side(watermark, [right], side='right', padding=280)
